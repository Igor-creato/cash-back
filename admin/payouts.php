...
<?php
    ...
    public function handle_update_payout_request(): void {
        ...
        // параллельная обработка ещё идёт. Старые клиенты без request_id работают как раньше.
        $idem_scope      = 'admin_payout_update';
        ...
        // Уведомление пользователю при переходе статуса в 'failed' (pending → available).
        // 'declined' не уведомляем — там pending → frozen, деньги не возвращаются.
        if (
            isset($update_data['status'])
            && 'failed' === $update_data['status']
            && isset($old_status)
            && 'failed' !== $old_status
            && !empty($payout_request['user_id'])
            && !empty($payout_request['total_amount'])
        ) {
            $refund_reason = $update_data['fail_reason'] ?? 'admin_cancel';
            if ('' === trim((string) $refund_reason)) {
                $refund_reason = 'admin_cancel';
            }
            do_action(
                'cashback_notification_payout_refunded',
                $payout_id,
                (int) $payout_request['user_id'],
                (string) $payout_request['total_amount'],
        ...
        // Убираем шифротекст И plaintext-реквизиты из ответа — не нужно отправлять в браузер.
        // Для просмотра реквизитов используется отдельный защищённый flow handle_decrypt_payout_details().
        unset($updated_payout_data['encrypted_details'], $updated_payout_data['payout_account']);

        $response_data = array(
            'payout_data' => $updated_payout_data,
        );

        // Фиксируем результат для идемпотентного retry (TTL = DEFAULT_TTL из helper'а).
        if ($idem_request_id !== '') {
    ...
    private function update_user_balance_on_payout( int $payout_id, bool $in_transaction = false, int $user_id = 0, string $amount = '' ): bool {
        ...
        try {
            ...
            // Money-сравнение через Cashback_Money (F-4-001): BCMath под капотом,
            // с strict-валидацией canonical decimal-string на входе.
    ...
    /**
     * AJAX-обработчик ручного возврата средств по заявке, реквизиты которой
     * не расшифровываются (key_mismatch / повреждённый ciphertext).
     *
     * Доступен только пользователям с manage_options, проверяет nonce,
     * делегирует фактическую отмену в cancel_payout_with_refund() с
     * reason='manual_encryption_recovery'.
     *
     * См. ADR Группа 4b (F-1-002).
     */
    public function handle_manual_encryption_refund(): void {
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'cashback_manual_encryption_refund')) {
            wp_send_json_error(array( 'message' => __('Неверный nonce.', 'cashback-plugin') ));
            return;
        }
        ...
        $ok = $this->cancel_payout_with_refund(
            $payout_id,
        ...
        if (!$ok) {
            wp_send_json_error(array(
                'message' => __('Не удалось отменить заявку. Возможно, она уже в финальном статусе или не найдена.', 'cashback-plugin'),
            ));
            return;
        }
    ...
    /**
     * Публичный entry-point для отмены активной заявки с возвратом средств в available.
     *
     * Переиспользует существующий защищённый путь `update_user_balance_on_failed()`:
     * открывает TX, делает FOR UPDATE на заявке, возвращает pending → available,
     * пишет ledger (payout_cancel) и audit_log. Отказывает на финальных статусах.
     *
     * Используется для:
     *   - Recovery-flow при утере ключа шифрования (batch-отмена waiting заявок).
     *   - Ручного возврата админом processing заявок, реквизиты которых не расшифровать.
     *
     * См. ADR Группа 4b, finding F-1-002.
     *
     * @param int    $payout_id  ID заявки на выплату.
     * @param string $reason     Причина отмены (для audit-log и fail_reason).
     * @param int    $actor_id   ID админа/юзера, инициировавшего отмену (0 = системный recovery-job).
     * @return bool              true — заявка отменена и средства возвращены (или уже были возвращены).
     */
    public function cancel_payout_with_refund( int $payout_id, string $reason, int $actor_id = 0 ): bool {
        global $wpdb;
        ...
        try {
            $row = $wpdb->get_row(
                $wpdb->prepare(
                    'SELECT id, user_id, total_amount, status, refunded_at
                     FROM %i
                     WHERE id = %d
                     FOR UPDATE',
                    $this->table_name,
            ...
            // Отказ на финальных статусах — их трогать нельзя (+триггеры БД тоже заблокируют).
            if (in_array($status, array( 'paid', 'failed', 'declined' ), true)) {
                $wpdb->query('ROLLBACK');
                return false;
            }

            // Idempotency: если refunded_at уже заполнен — считаем отмену успешной,
            // повторно баланс не трогаем.
            ...
            // pending → available + ledger payout_cancel + refunded_at (в текущей TX).
            $refunded = $this->update_user_balance_on_failed($payout_id, true, $user_id, $amount);
            if (!$refunded) {
                $wpdb->query('ROLLBACK');
                return false;
            }

            // Переводим статус → failed с причиной.
            ...
            // Audit-log — после COMMIT (аудит не должен ломать основной поток).
            if (class_exists('Cashback_Encryption') && method_exists('Cashback_Encryption', 'write_audit_log')) {
                try {
                    Cashback_Encryption::write_audit_log(
                        'payout_cancelled_encryption_recovery',
                        $actor_id,
                        'payout_request',
                        $payout_id,
                        array(
            ...
            $this->log_info("Payout #{$payout_id} cancelled with refund. Reason: {$reason}, amount: {$amount}");

            // Уведомление пользователю о возврате средств (после COMMIT, чтобы не ловить откат).
            do_action(
                'cashback_notification_payout_refunded',
                $payout_id,
                $user_id,
            ...
            $wpdb->query('ROLLBACK');
            $this->log_error("cancel_payout_with_refund failed for payout #{$payout_id}: " . $e->getMessage());
            return false;
        }
    }
    ...
    private function update_user_balance_on_failed( int $payout_id, bool $in_transaction = false, int $user_id = 0, string $amount = '' ): bool {
        ...
        try {
            ...
            $refund_time   = Cashback_Time::now_mysql();
            $update_result = $wpdb->update(
                $this->table_name,
                array(
    ...
    public function handle_get_payout_request(): void {
        ...
        // Убираем шифротекст И plaintext-реквизиты из ответа — не нужно отправлять в браузер.
        // Для просмотра реквизитов используется отдельный защищённый flow handle_decrypt_payout_details().
        unset($payout_data['encrypted_details'], $payout_data['payout_account']);
    ...
    public function handle_decrypt_payout_details(): void {
        ...
        // Fail-fast по fingerprint: если ключ не совпадает с ранее зафиксированным,
        // реквизиты уже нерасшифруемы. Показываем админу warning с кнопкой manual refund.
        // См. ADR Группа 4b (F-1-002).
        if (!empty($payout['encrypted_details'])
            && class_exists('Cashback_Encryption_Recovery')
            && Cashback_Encryption_Recovery::is_key_mismatch()
        ) {
            wp_send_json_error(array(
                'code'              => 'key_mismatch',
                'message'           => __('Ключ шифрования изменён. Данные невозможно расшифровать. Вернуть средства пользователю вручную?', 'cashback-plugin'),
        ...
        if (!empty($payout['encrypted_details']) && class_exists('Cashback_Encryption') && Cashback_Encryption::is_configured()) {
            try {
                ...
                // Сбой расшифровки при валидном (казалось бы) fingerprint — скорее всего
                // повреждённый ciphertext. Семантически идентично key_mismatch: реквизиты
                // не восстановить, предлагаем manual refund.
                wp_send_json_error(array(
                    'code'              => 'key_mismatch',
                    'message'           => __('Не удалось расшифровать данные. Вернуть средства пользователю вручную?', 'cashback-plugin'),
                    'can_manual_refund' => true,
                ));
                return;
            }
    ...
    public function handle_verify_payout_balance(): void {
        ...
        foreach ($type_labels as $type => $label) {
            $sum = $sums[ $type ] ?? '0.00';
            $cnt = $counts[ $type ] ?? 0;
            if ($cnt > 0 || ! Cashback_Money::from_db_value($sum)->is_zero()) {
                $operations_summary[] = array(
                    'label' => $label,
                    'count' => $cnt,
