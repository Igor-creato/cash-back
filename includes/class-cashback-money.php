...
final class Cashback_Money {
	 ...
	 *
	 * @param mixed $value Входное значение (ожидается string).
	...
	public static function from_string( $value ): self {
		if ( ! is_string( $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'Cashback_Money::from_string expects string, got ' . gettype( $value ) )
			);
		}
		if ( ! preg_match( self::INPUT_REGEX, $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'Cashback_Money::from_string rejects non-canonical money input: ' . $value )
			);
		}
	 ...
	 *
	 * @param mixed $value Значение из БД (ожидается string от wpdb).
	 * @throws InvalidArgumentException Если значение не string или не canonical money.
	 */
	public static function from_db_value( $value ): self {
		if ( ! is_string( $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'Cashback_Money::from_db_value expects string (DECIMAL), got ' . gettype( $value ) )
			);
		}
		if ( ! preg_match( self::INPUT_REGEX, $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'Cashback_Money::from_db_value rejects non-canonical DB value: ' . $value )
			);
		}
	...
	public function mul_rate( string $rate_percent ): self {
		if ( ! preg_match( self::RATE_REGEX, $rate_percent ) ) {
			throw new InvalidArgumentException(
				esc_html( 'Cashback_Money::mul_rate rejects non-numeric rate: ' . $rate_percent )
			);
		}
	 ...
	 *
	 * @param mixed $value Значение для валидации (ожидается canonical string).
	 * @throws InvalidArgumentException Если значение не canonical money-string.
	 */
	public static function assert_money_string( $value ): void {
		if ( ! is_string( $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'assert_money_string expects canonical string, got ' . gettype( $value ) )
			);
		}
		if ( ! preg_match( self::CANONICAL_REGEX, $value ) ) {
			throw new InvalidArgumentException(
				esc_html( 'assert_money_string rejects non-canonical money string: ' . $value )
			);
		}
	}
