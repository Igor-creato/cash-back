...
class Cashback_Advcake_Adapter extends Cashback_Network_Adapter_Base {
    ...
    // phpcs:disable Squiz.Functions.MultiLineFunctionDeclaration.ContentAfterBrace,Squiz.ControlStructures.ControlSignature.NewlineAfterOpenBrace,Generic.Formatting.DisallowMultipleStatements.SameLine,Generic.Functions.FunctionCallArgumentSpacing.TooMuchSpaceAfterComma,NormalizedArrays.Arrays.ArrayBraceSpacing.SpaceAfterArrayOpenerSingleLine,Universal.WhiteSpace.CommaSpacing.TooMuchSpaceAfter,NormalizedArrays.Arrays.CommaAfterLast.FoundSingleLine,NormalizedArrays.Arrays.ArrayBraceSpacing.SpaceBeforeArrayCloserSingleLine,PSR2.Classes.ClassDeclaration.CloseBraceAfterBody
    /**
     * Подтянуть «% подтверждения заказов» одного оффера через `/stat`.
     *
     * Контракт совпадает с `Cashback_Admitad_Adapter::fetch_campaign_by_id()` —
     * это де-факто интерфейс, на который смотрят
     * `Cashback_Shop_Rate_Of_Approve_Refresher` и
     * `Cashback_CPA_Approval_Rate_Provider`. Возврат:
     *   - success=true + campaign=['rate_of_approve' => float] — есть свежий сигнал;
     *   - success=true + campaign=null — оффер не найден / нет заказов в окне (refresher удалит мету → UI «нет данных»);
     *   - success=false + error  — транзитная/токен/сетевая ошибка.
     *
     * Endpoint Advcake: `GET {api_base_url}/stat?pass={token}&offer_id={N}&days=30&type=json`.
     * Поле `ar` приходит float в [0..1] (например 0.0976 = 9.76%); масштабируем
     * в проценты [0..100], как у Admitad. Окно `days` по умолчанию 30 — совпадает
     * с соседним локальным блоком «% подтверждения заказов магазином (30 дней)»
     * в редакторе товара. Filterable через `cashback_advcake_stat_days`.
     * `orders_total = 0` (или пустой `data: []`) трактуется как «недостаточно
     * данных» — отдаём success+null (не false-0%), filter `cashback_advcake_stat_min_orders`.
     *
     * @param array $credentials
     * @param array $network_config
     * @param int                  $retry_5xx_attempt  Bounded counter (макс. 2)
     * @param int                  $retry_429_attempt  Bounded counter for 429 (макс. 2)
     * @return array
     */
    public function fetch_campaign_by_id( array $credentials, array $network_config, string $campaign_id, int $retry_5xx_attempt = 0, int $retry_429_attempt = 0 ): array {
        ...
        if ($token === null) {
            return array(
                'success'  => false,
                'campaign' => null,
                'error'    => $this->last_token_error !== '' ? $this->last_token_error : 'Не удалось получить токен Advcake',
            );
        }
        ...
        $days_default = 30;
        $days         = (int) apply_filters('cashback_advcake_stat_days', $days_default, $campaign_id);
        if ($days < 1 || $days > 365) {
            $days = $days_default;
        }

        $min_orders = (int) apply_filters('cashback_advcake_stat_min_orders', 1, $campaign_id);
        if ($min_orders < 0) {
            $min_orders = 0;
        }
        ...
        if (is_wp_error($response)) {
            return array(
                'success'  => false,
                'campaign' => null,
                'error'    => 'HTTP error: ' . $this->redact_secret_tokens((string) $response->get_error_message()),
            );
        }
        ...
        // 401/403 — токен Advcake невалиден/отозван. Никакого refresh-handshake
        // у Advcake нет (api_key статичный), поэтому invalidate-noop — терминальная ошибка.
        if ($code === 401 || $code === 403) {
            return array(
                'success'  => false,
                'campaign' => null,
                'error'    => "HTTP {$code}: токен Advcake отвергнут на /stat — обновите api_key в Настройках API.",
            );
        }

        // 429 — явный transient rate-limit. Даже если endpoint не документирован,
        // безопаснее сделать bounded retry как у Admitad, чем жёстко фейлить цикл.
        if ($code === 429 && $retry_429_attempt < 2) {
            $delay = $this->parse_retry_after_seconds($response, (string) $body);
            $delay = (int) apply_filters(
            ...
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional plugin diagnostic logging.
            error_log("Cashback Advcake: HTTP 429 on /stat for offer_id={$campaign_id}, retry attempt " . ( $retry_429_attempt + 1 ) . ' of 2');
            return $this->fetch_campaign_by_id($credentials, $network_config, $campaign_id, $retry_5xx_attempt, $retry_429_attempt + 1);
        }

        // code=0 (network/proxy anomaly without WP_Error) и 5xx — transient.
        // До 2 retry с backoff; счётчик bounded против infinite-loop'а.
        if (($code === 0 || ( $code >= 500 && $code < 600 )) && $retry_5xx_attempt < 2) {
            ...
            // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional plugin diagnostic logging.
            error_log("Cashback Advcake: HTTP {$code} on /stat?offer_id={$campaign_id}, retry attempt {$next_attempt} of 2");
            return $this->fetch_campaign_by_id($credentials, $network_config, $campaign_id, $next_attempt, $retry_429_attempt);
        }

        // 404 — у Advcake `/stat` обычно отвечает 200 с пустым `data: []`,
        // но если LB/прокси отдаст 404 — трактуем как «оффера нет» (success+null).
        if ($code === 404) {
            return array(
                'success'  => true,
                'campaign' => null,
        ...
        if ($code !== 200) {
            return array(
                'success'  => false,
                'campaign' => null,
                'error'    => "HTTP {$code} /stat: " . $this->safe_error_summary($body),
            );
        }
        ...
        if (!is_array($decoded)) {
            return array(
                'success'  => false,
                'campaign' => null,
                'error'    => 'Malformed JSON in /stat response',
            );
        }

        // Publisher API: `success: false` + `error` — отказ на уровне приложения
        // (например, токен не имеет доступа к статистике). Делаем терминальной ошибкой.
        if (array_key_exists('success', $decoded) && $this->is_false_api_flag($decoded['success'])) {
$api_error = isset($decoded['error']) ? (string) $decoded['error'] : 'неизвестная ошибка';
return array( 'success'  => false, 'campaign' => null, 'error'    => '/stat API success=false: ' . $api_error );
        }

        $data = isset($decoded['data']) && is_array($decoded['data']) ? $decoded['data'] : array();

        // Пустой data (offer_id не найден ИЛИ нет заказов в окне) → success+null
        // (как Admitad 404). Refresher удалит post_meta → UI покажет «нет данных от сети».
        if ($data === array()) {
            return array(
                'success'  => true,
                'campaign' => null,
        ...
        // Берём первый row — на `offer_id=N` Advcake возвращает ровно один (или ноль) элементов.
        $row = $data[0] ?? null;
        if (!is_array($row)) {
            return array(
                'success'  => true,
        ...
        $orders_total = isset($row['orders_total']) && is_numeric($row['orders_total']) ? (int) $row['orders_total'] : 0;
        if ($orders_total < $min_orders) {
            // Недостаточно заказов в окне — `ar` ненадёжен (или 0/null).
            return array(
                'success'  => true,
                'campaign' => null,
                'error'    => null,
        ...
        $rate = $this->extract_rate_of_approve_from_stat($row);
        if ($rate === null) {
            return array(
                'success'  => true,
                'campaign' => null,
        ...
        return array(
            'success'  => true,
            'campaign' => array( 'rate_of_approve' => $rate ),
            'error'    => null,
        );
    }
    ...
    /**
     * Извлечь approval rate из row `/stat` Advcake в проценты [0..100].
     *
     * Advcake отдаёт поле `ar` в долях единицы (0..1, например 0.0976 = 9.76%);
     * масштабируем в проценты, чтобы совпало с семантикой Admitad
     * (`rate_of_approve` уже в [0..100]). Выходное значение округляется до
     * 2 знаков — UI читает с тем же `number_format(.., 1)`.
     *
     * Имя поля filterable через `cashback_advcake_stat_rate_fields` — на случай
     * если у наследника адаптера поле называется иначе. Значения вне [0..1]
     * (включая отрицательные и numeric-NaN-строки) → null: явно «нет валидного
     * сигнала», caller (Refresher) удалит post_meta.
     *
     * @param array $row
     * @return float|null
     */
    private function extract_rate_of_approve_from_stat( array $row ): ?float {
        $candidates = array( 'ar' );
        /** @var array<int, string> $candidates */
        $candidates = (array) apply_filters( 'cashback_advcake_stat_rate_fields', $candidates, $row );
        ...
        foreach ( $candidates as $key ) {
            ...
            $raw = (float) $row[ $key ];
            if ( $raw < 0.0 || $raw > 1.0 ) {
                // Защита от формата 0..100 в будущем — отдаём null вместо ложного 0.
                // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional plugin diagnostic logging.
                error_log( 'Cashback Advcake: /stat field "' . $key . '" out of [0..1]: ' . $raw );
                continue;
            }
    ...
    /**
     * Нормализовать ложные app-level флаги `success`.
     *
     * API документирует boolean, но defensive-обработка строкового "false"
     * не даёт замаскировать отказ как success+null.
     *
     * @param mixed $value
     */
    private function is_false_api_flag( $value ): bool {
    ...
    /**
     * Извлечь retry-after для 429. Header имеет приоритет, затем body.
     *
     * @param mixed $response
     */
    private function parse_retry_after_seconds( $response, string $body ): int {
        ...
        if (preg_match('/retry[- ]after["=: ]+(\d+)/i', $body, $m)) {
            return (int) $m[1];
        }
