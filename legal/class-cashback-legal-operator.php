...
class Cashback_Legal_Operator {
    ...
    public static function render_placeholders( string $content, array $extra = array() ): string {
        ...
        // `{{#if_contact_phone}}…{{/if_contact_phone}}`: если соответствующее поле
        // пустое (Самозанятый — нет ОГРН/КПП; ИП — нет КПП; телефон опционален —
        // не публикуется при пустом значении, чтобы не получать спам), блок удаляется
        // целиком. Иначе раскрывается без обёрток, плейсхолдеры внутри подставляются
        // обычным strtr ниже.
        $content = (string) preg_replace_callback(
            '/\{\{#if_(ogrn|kpp|contact_phone)\}\}(.*?)\{\{\/if_\1\}\}/s',
