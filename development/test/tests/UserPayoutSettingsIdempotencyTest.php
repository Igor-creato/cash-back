...
/**
 * Контрактные тесты для `cashback-withdrawal.php::save_payout_settings` —
 * server-side дедуп request_id (Группа 5 ADR, follow-up из E2E прогона
 * 2026-04-29 — finding A6-2: retry бил 3-сохранения-в-24ч квоту без дедупа).
 */
#[Group('idempotency')]
#[Group('group-5')]
final class UserPayoutSettingsIdempotencyTest extends TestCase
{
    private const HANDLER_FILE = __DIR__ . '/../../../cashback-withdrawal.php';
    private const JS_FILE      = __DIR__ . '/../../../assets/js/cashback-withdrawal.js';

    private function method_body(): string
    {
        $src = file_get_contents(self::HANDLER_FILE);
        $this->assertIsString($src);
        if (!preg_match('/public function save_payout_settings\s*\([^)]*\)[^{]*\{/', $src, $m, PREG_OFFSET_CAPTURE)) {
            $this->fail('save_payout_settings() not found');
        }
        $start = $m[0][1];
    ...
    {
        $this->assertStringContainsString("'user_payout_settings'", $this->method_body());
    }
    ...
    public function test_handler_releases_claim_on_error_paths(): void
    {
        ...
        // Минимум 2: один на ошибку валидации после claim, один на DB-fail.
    ...
    public function test_js_sends_request_id_for_save_payout_settings(): void
    {
        $js = file_get_contents(self::JS_FILE);
        $this->assertIsString($js);

        // settingsData должен содержать request_id, генерируемый через
        // generateIdempotencyKey() (UUID v4 через crypto.randomUUID).
        $this->assertMatchesRegularExpression(
            "/action:\s*'save_payout_settings'[\s\S]{0,500}request_id:\s*generateIdempotencyKey\(\)/",
            $js
        );
    }
}
