/*
 * Cashback admin — vanilla JS модал combined approve+tx (Session 4-bis,
 * P0.1 F-S7-NO-MANUAL-CREDIT). Заменяет legacy approve-кнопку
 * (data-action="approved" → claims_admin_transition) на атомарный
 * UPDATE+INSERT через `cashback_claim_approve_with_tx`.
 *
 * Контракт:
 *  - Кнопка-триггер: <button class="cashback-claim-approve-tx" data-claim-id="N">.
 *    Внутри AJAX-loaded #claim-detail-body — поэтому делегат на document.
 *  - Разметка модала рендерится PHP'ом в footer'е страницы — JS только биндит
 *    обработчики и шлёт submit. Preview-load НЕ делается (claim уже виден на странице).
 *  - cashbackClaimApproveTx (от wp_localize_script): { ajaxUrl, nonce, i18n }.
 *
 * AJAX:
 *  - action=cashback_claim_approve_with_tx → атомарно UPDATE status='approved'
 *    + INSERT в cashback_transactions в одной TX. Server-side дедуп по
 *    request_id (UUIDv4 из crypto.randomUUID).
 */
( function () {
	'use strict';

	var cfg = window.cashbackClaimApproveTx || {};
	...
	function findElements() {
		...
		var backdrop = document.getElementById( 'cashback-claim-approve-tx-backdrop' );
		if ( ! backdrop ) {
			return null;
		}
		...
		els = {
			...
			comission: dialog.querySelector( '#cashback-claim-approve-tx-comission' ),
			fundsReady: dialog.querySelector( '#cashback-claim-approve-tx-funds-ready' )
		};
		return els;
	}
	...
	function openModal( claimId ) {
		...
		// В отличие от stuck-claim flow preview-load не нужен — claim
		// уже отрендерен в #claim-detail-body. Сразу показываем форму.
	...
	function submit() {
		...
		body.append( 'action', 'cashback_claim_approve_with_tx' );
		...
		fetch( cfg.ajaxUrl, {
			...
			.then( function ( res ) {
				if ( res.ok && res.json && res.json.success ) {
					...
					// 1.5с чтобы админ увидел reference_id, потом reload —
					// claim перейдёт в approved и tx появится в истории.
					setTimeout( function () {
			...
			.catch( function () {
				...
				e.submit.disabled = false;
				e.cancel.disabled = false;
				// Сетевую ошибку retry'ем тем же request_id — server-side кэш
				// отдаст результат если запрос всё-таки дошёл.
			} );
	}

	// Делегированный listener — кнопка-триггер находится внутри AJAX-loaded
	// #claim-detail-body, поэтому биндимся на document.
	document.addEventListener( 'click', function ( e ) {
		var btn = e.target.closest && e.target.closest( '.cashback-claim-approve-tx' );
		if ( ! btn ) {
			return;
		}
		e.preventDefault();
